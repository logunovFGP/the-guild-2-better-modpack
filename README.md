...
| `python ai_telemetry.py [log] --findings` | **Read this first after a play session.** Every defect six sessions have taught us, as a check that runs itself: one ERROR / WARN / NOTE line each with a literal pointer to the file to open, and the date a thing was fixed so a finding that should be impossible reads *inspect this*, not *known issue*. Covers the library Includes, Lua errors, replay drift, measures that cancel their own start, barren `BloodFeud` entries, HTN methods that never applied and steps planned but never picked, the feud cart, the market `nowhere` column, hand-overs, `BuyWorkshop` cadence, and knobs still at their testing value. Add to `CHECKS` whenever a session teaches us something new - and that is enforced, not requested: `--selftest` fails if a pointer names a file, function or knob that is not in the tree, or if the Lua emits a `::TWP::` channel nothing here parses. It found two on its first run. |
