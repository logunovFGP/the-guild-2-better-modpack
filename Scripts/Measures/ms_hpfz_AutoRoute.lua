...
function Init()
	...
	local IntervalLabel = "_TRADEROUTE_INITIATE_INTERVAL_+0"
	...
	repeat -- building overview, building select and send out
		local optionen = { -- TODO list current buildings as buttons or submenu
											"@B[1,@L_TRADEROUTE_INITIATE_OPTION_+2,]", -- N�chstes Geb�ude
											...
											"@B[3,@L_TRADEROUTE_INITIATE_OPTION_+3,]", -- Aussenden
		...
		vorgang = MsgBox("","Owner","@P"..optionen2,"@L_TRADEROUTE_INITIATE_HEAD_+0","_TRADEROUTE_INITIATE_BODY_+0", SaleThreshold, IntervalLabel)	 -- Vorgang w�hlen
		...
		elseif vorgang == 5 then
			local ret = MsgBox("", "Owner", 
				"@P@B[0,@L_TRADEROUTE_SALETHRESHOLD_+0,]"..
				"@B[75,@L_TRADEROUTE_SALETHRESHOLD_+1,]"..
				"@B[100,@L_TRADEROUTE_SALETHRESHOLD_+2,]"..
				"@B[125,@L_TRADEROUTE_SALETHRESHOLD_+3,]"..
				"@B[150,@L_TRADEROUTE_SALETHRESHOLD_+4,]",
				"@L_TRADEROUTE_SALETHRESHOLD_HEAD_+0",
				"@L_TRADEROUTE_SALETHRESHOLD_BODY_+0",
				SaleThreshold)
			if ret ~= "C" then
				SaleThreshold = ret
			end 
		elseif vorgang == 6 then
			local ret = MsgBox("","Owner","@P@B[0,@L_TRADEROUTE_INITIATE_INTERVAL_+0,]".."@B[1,@L_TRADEROUTE_INITIATE_INTERVAL_+1,]".."@B[6,@L_TRADEROUTE_INITIATE_INTERVAL_+2,]".."@B[12,@L_TRADEROUTE_INITIATE_INTERVAL_+3,]","@L_TRADEROUTE_INTERVAL_HEAD_+0","@L_TRADEROUTE_INTERVAL_BODY_+0")
			if ret == 0 or ret == 1 then
				IntervalLabel = "_TRADEROUTE_INITIATE_INTERVAL_+"..ret
				Interval = ret
			elseif ret == 6 then
				IntervalLabel = "_TRADEROUTE_INITIATE_INTERVAL_+2"
				Interval = ret
			elseif ret == 12 then
				IntervalLabel = "_TRADEROUTE_INITIATE_INTERVAL_+3"
				Interval = ret
			end				
...
function ActionSelect(ort, CurrentItems)
	...
	local optionen = {"@B[1,@L_TRADEROUTE_ACTIONSELECT_OPTION_+1,]", -- Einladen
										...
										"@B[3,@L_TRADEROUTE_ACTIONSELECT_OPTION_+3,]",--Lager auff�llen
										...
										"@B[C,@L_TRADEROUTE_ACTIONSELECT_OPTION_+0,]"}-- Zur�ck
	...
	repeat 
		...
		if GetDynastyID("") == GetDynastyID(ort) then
			optionen2 = optionen2..optionen[3]--Lager auff�llen
		end
		if not wichtig then
			optionen2 = optionen2..optionen[4] -- als wichtig markieren
			imp = "_TRADEROUTE_ACTIONSELECT_BODY_+2"
		else
			optionen2 = optionen2..optionen[5] -- als nicht wichtig markieren
			imp = "_TRADEROUTE_ACTIONSELECT_BODY_+1"
		end
		optionen2 = optionen2..optionen[6]--Zur�ck
		vorgang = MsgBox("","Owner","@P"..optionen2,"@L_TRADEROUTE_ACTIONSELECT_HEAD_+0","@L_TRADEROUTE_ACTIONSELECT_BODY_+0", GetID(ort), imp)	-- Vorgang w�hlen

		if vorgang == "C" then
			-- Zur�ck zur Geb�udeauswahl
			return ActionCount, Actions, wichtig
			
		elseif vorgang == 1 then -- Einladen
			was = 1
			wovon, wieviel = ms_hpfz_autoroute_ChooseItemsToLoad("", ort)
		...
		elseif vorgang == 3 then -- Lager auff�llen
			was = 3
			wovon, wieviel = ms_hpfz_autoroute_InitUnload(ort, true, CurrentItems)
			-- kein �ndern des Karrenlagers, da ggf. gar keine Waren abgeladen werden
...
-- zeigt einen Dialog mit einem Button pro Station und einem Zur�ck-Button 
function ShowRoute(StationCount, Stations)
	...
	local vorgang = MsgBox("","Owner",buttons,"@L_TRADEROUTE_INITIATE_HEAD_+0","")	-- Vorgang w�hlen
...
--		local bodytext = "@L_TRADEROUTE_TYPESELECT_BODY_+1"
--		if isRestock then
--			bodytext = "@L_TRADEROUTE_TYPESELECT_BODY_+2"
--		end	
--		drinWahl = MsgBox("","Owner","@P"..karStuf,"@L_TRADEROUTE_TYPESELECT_HEAD_+0", bodytext, GetID(station))
		drinWahl = InitData(
		...
		if isRestock then
			...
			bodytext = "@L_TRADEROUTE_COUNTSELECT_BODY_+1"
		end	
		drinMeng = MsgBox("","Owner","@P"..mengStuf,"@L_TRADEROUTE_COUNTSELECT_HEAD_+0",bodytext,GetID(station), ItemGetLabel(drinWahl,false))
		...
		for s = 1, StationCount do
			...
			repeat
				...
				for i = 1, Stations[s][2] do
					...
					if Action == 1 then 
						------ Einladen oder Karren auff�llen ----
	...
	for s = 1, StationCount do
		...
		if Stations[s][2] >= 1 then
			for a = 1, Stations[s][2] do
				...
				SetData("S"..s.."Action"..a, Action[1]) -- Aktion: 1=Einladen, 2=Ausladen, 3=Auff�llen
...
function Unload(Station, Type, Count, Threshold)
	...
	if Threshold > 0 and (BuildingGetClass(Station) == GL_BUILDING_CLASS_MARKET) 
			and GetHomeBuilding("", "Business") 
			and BuildingGetOwner("Business", "MyBoss") 
...
function CheckWarning(NumberOfTries, WarningCount, Station)
	if (WarningCount > 0) and (NumberOfTries >= WarningCount*2) then
		local ret = MsgNews("", "", 
			"@P@B[C,@L_TRADEROUTE_WARNING_OPTION_+0,]".."@B[1,@L_TRADEROUTE_WARNING_OPTION_+1,]".."@B[2,@L_TRADEROUTE_WARNING_OPTION_+2,]".."@B[3,@L_TRADEROUTE_WARNING_OPTION_+3,]", 
			0,
			"production", 
			1, 
			"@L_TRADEROUTE_WARNING_HEAD_+0", 
			"@L_TRADEROUTE_WARNING_BODY_+0", 
