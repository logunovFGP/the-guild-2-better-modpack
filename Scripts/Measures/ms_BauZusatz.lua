...
function ArbeiterGen(wahl)
	...
	for l=1, nerv do
		local RandomSpawn = "Walledge"..l
		if l > 4 then
			RandomSpawn = "Entry1"
		end
		if not GetLocatorByName("Destination", RandomSpawn, "SpawnPos") then
			CopyAlias("Destination", "SpawnPos")
		end
		SimCreate(manstuff[check], "Destination", "SpawnPos", "BauArbeiter"..l)
