function Run()
	...
	while not HasData("LetsGo") do
		if chr_CityFindCrowdedPlace("MyCity", "", true, "PlayPos") then
			f_MoveTo("", "PlayPos")
			Sleep(1)
			...
			if not GetOutdoorLocator("MapExit3", 1, "MapExit") then
				...
				for r =0, citys-1 do
					...
					if CityIsKontor("Stadt"..k) == false then
						CityGetRandomBuilding("Stadt"..k,-1,18,-1,-1,FILTER_IGNORE,"MapExit")
						break
					end
				end			
			end
	...
	Sleep(4)
	-- wait for despawning until all kids are gone
	while true do
		local KidsFilter = "__F( (Object.GetObjectsByRadius(Sim)==1500)AND NOT(Object.MinAge(15)))"
		local NumKids = Find("", KidsFilter, "RatBoy", -1)
		if NumKids >0 then
