function Run()
	...
	if (GetProperty("Actor", "jewellery")==nil) or (GetProperty("Actor", "jewellery")==8) then
		...
		if GetSettlement("", "my_settlement") then
			if (gameplayformulas_CheckPublicBuilding("my_settlement", GL_BUILDING_TYPE_GUILDHOUSE)[1]>0) and CityGetRandomBuilding("my_settlement", -1, GL_BUILDING_TYPE_BANK, -1, -1, FILTER_IGNORE, "GuildHouse") then
